...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(t('auth.login.success', 'Login successful! Redirecting...'));
      navigate('/dashboard');
    } catch (err: any) {
      ...
      switch (err.code) {
        ...
        case 'auth/wrong-password':
          setError(t('auth.error.wrong.password', 'Incorrect password'));
          break;
        case 'auth/invalid-email':
          setError(t('auth.error.invalid.email', 'Please enter a valid email address'));
          break;
        case 'auth/too-many-requests':
          setError(t('auth.error.too.many.requests', 'Too many failed attempts. Please try again later'));
          break;
        default:
          setError(t('auth.error.login.failed', 'Login failed. Please check your credentials and try again'));
      }
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      switch (err.code) {
        case 'auth/popup-closed-by-user':
          setError(t('auth.error.popup.closed', 'Sign-in popup was closed. Please try again'));
          break;
        case 'auth/popup-blocked':
          setError(t('auth.error.popup.blocked', 'Sign-in popup was blocked. Please allow popups and try again'));
          break;
        case 'auth/cancelled-popup-request':
          setError(t('auth.error.popup.cancelled', 'Sign-in was cancelled. Please try again'));
          break;
        case 'auth/network-request-failed':
          setError(t('auth.error.network', 'Network error. Please check your connection and try again'));
          break;
        default:
          setError(t('auth.error.google.failed', 'Google sign-in failed. Please try again'));
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="mt-6">
              ...
              <div className="mt-6">
                <button
                  ...
                  disabled={loading}
                  className="w-full inline-flex justify-center py-3 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  ...
                  {t('auth.google', 'Login with Google')}
                </button>
              </div>
            </div>
          </form>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600">
              {t('auth.no.account', "Don't have an account?")}{' '}
              <Link
                to="/signup"
                className="font-medium text-indigo-600 hover:text-indigo-500 transition-colors"
              >
